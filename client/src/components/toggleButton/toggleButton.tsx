    ...
    return (
        <label className="flex items-center cursor-pointer">
            ...
            <div className="relative">
                ...
                />
                <div
                    className={`block w-12 lg:w-14 lg:h-8 h-6 border-[1px] rounded-full ${toggle ? 'bg-white' : 'bg-bgColor border-white'
                        }`}
                ></div>
                <div
                    className={`absolute left-1 top-1 ${toggle ? 'bg-bgColor' : 'bg-white'} w-4 lg:w-6 h-4 lg:h-6 rounded-full transition-transform duration-300 ease-in-out ${toggle ? 'transform translate-x-6' : ''
                        }`}
                ></div>
            </div>
