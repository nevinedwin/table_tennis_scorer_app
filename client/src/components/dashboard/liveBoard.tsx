      ...
      <div className="w-full text-center">
        {isLive ? (
          data?.winner ? (
            ...
            <p className="font-bold text-xl lg:text-2xl animate-pulse text-primary">
            <span className="bg-red-500 text-white px-2 rounded-full">Live</span>
          </p>
          )
        ) : isMatch ? (
      ...
      <div className="relative flex w-full h-full flex-col justify-center items-center">
        ...
        <div className="w-full lg:pt-[1%] flex flex-col">
          <div className="flex justify-center items-center">
            <p className="border border-borderColor lg:border-none px-4 text-xl lg:text-2xl font-bold">
              Match {data?.matchNumber}
            </p>
          </div>
          <div className="bg-gray-900 rounded-lg p-8 w-full h-auto shadow-lg">
          <div className="flex px-2">
            <div className="border w-[33%]  border-borderColor lg:border-none flex-1 text-end  pr-10  px-1 pt-2 lg:pt-5 text-md lg:text-xxl overflow-hidden whitespace-nowrap text-ellipsis">
              {data?.team1Name}
            </div>
            <div className="border w-[33%]  border-borderColor lg:border-none flex-1 text-start pl-10  px-1 pt-2 lg:pt-5 text-md lg:text-xxl overflow-hidden whitespace-nowrap text-ellipsis">
              {data?.team2Name}
            </div>
          </div>
          <div className="flex box-border px-2 justify-center">
            ...
            <div
              onClick={() =>
                handleClick(data?.matchId as string, data?.team1Id as string)
              }
              className="w-[33%] lg:w-[13%] h-[100%] lg:h-[100%] border border-borderColor  p-1"
            >
              <LiveScoreBoard
                score={
                  data?.currentSet === 1
                    ...
                    : data?.currentSet === 2
                    ? data?.team1Set2Score
                    : (data?.team1Set3Score as number)
                }
              />
            </div>
            <div
              onClick={() =>
                handleClick(data?.matchId as string, data?.team2Id as string)
              }
              className="w-[33%] lg:w-[13%] h-[100%] lg:h-[100%] border border-borderColor  p-1"
            >
              <LiveScoreBoard
          ...
          <div className="w-full pt-5 lg:pt-[1%] flex justify-center  items-center gap-5">
            {data?.set1Winner && (
              <div className="">
                <ScoreChip
                  setNumber={1}
                  team1Score={data?.team1Set1Score}
                  team2Score={data?.team2Set1Score}
                />
              </div>
            )}
            {data?.set2Winner && (
              <div className="">
                <ScoreChip
                  setNumber={2}
                  team1Score={data?.team1Set2Score as number}
                  team2Score={data?.team2Set2Score as number}
                />
              </div>
            )}
            {data?.set3Winner && (
              <div className="">
                <ScoreChip
                  setNumber={3}
                  team1Score={data?.team1Set3Score as number}
                  team2Score={data?.team2Set3Score as number}
                />
              </div>
            )}
          </div>
          {user?.role === UserRole.SUPER_ADMIN && (
            <div className="flex items-center justify-center pt-5 lg:pt-[1%] lg:gap-2 gap-4">
              ...
              <button
                onClick={() => handleRemove(data?.matchId as string)}
